...
- `tenants.settings.nip05_domain` (database): Optional per-tenant override for profile `nip05` domain; falls back to `tenants.domain` when unset. Backfill this per tenant before changing routing domains to avoid silent NIP-05 identifier cutovers.
